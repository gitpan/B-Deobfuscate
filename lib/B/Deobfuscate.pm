...
sub lookup_sigil {
    ...
    return '@'
        if $rv =~ /(?:gvav|padav|av2arylen|rv2av|aelemfast|aelem|aslice)$/;

    return '%'
        if $rv =~ /(?:padhv|rv2hv|helem|hslice)$/;

    return '&'
        if $rv =~ /rv2cv$/;

    return ''
        if $rv =~ /(?:gv|gelem|rv2gv)$/;
